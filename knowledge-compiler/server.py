...
@app.post("/api/node-qa")
async def node_qa(request: Request):
    """
    AI Q&A for a single knowledge node.
    Context: current node + all children title+content.
    Each question is independent (no multi-turn history).
    """
    ...
    prompt = f"""你是一位教育学助教，正在帮助学生理解教材知识点。
...
## 要求
- 严格基于上方节点内容回答，不要引入节点内容以外的知识
- 如果节点内容不足以回答，明确告知"当前节点内容未涉及此问题"
- 回答简洁，300字以内
- 用中文回答
"""
